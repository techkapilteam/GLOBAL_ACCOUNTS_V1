...
export class TdsJvComponent implements OnInit {
  ...
  //         this._commonService.getbranchname(),
  //         creditledger, monthYear, debitledger,
  //         this._commonService.getschemaname(),
  //         this._commonService.getBranchCode(),
  //         this._commonService.getCompanyCode()
  ...
  //           this.dataisempty = false;
  //           //    this.totaldebitamount = this.tdsJvDetailsGrid
  //           // .reduce((sum, c) => sum + parseFloat(c.debit_amount), 0);
  //           this.totalcreditamount = this.tdsJvDetailsGrid.
  //           reduce((sum:any, c:any) => sum + parseFloat(c.credit_amount), 0);

  //           this._AccountingTransactionsService.GetTDSJVDetailsDuplicateCheck(
  //             this._commonService.getbranchname(),
  //            debitledger,  monthYear,this._commonService.getschemaname(),this._commonService.getCompanyCode(),this._commonService.getBranchCode()
  ...
  const monthYear = this.MonthName?.toUpperCase() || '';
  ...
  this._AccountingTransactionsService.GettdsJVDetails(
    ...
    if (res && res.length > 0) {
      ...
      this.tdsJvDetailsGrid = res.map(item => ({
        ...item,
      ...
      this.totaldebitamount = this.tdsJvDetailsGrid.reduce(
        (sum:any, c:any) => sum + c.debit_amount, 0
      );
      this.totalcreditamount = this.tdsJvDetailsGrid.reduce(
        (sum:any, c:any) => sum + c.credit_amount, 0
      );
    ...
    console.error('Error fetching TDS JV Details:', error);
    this.tdsJvDetailsGrid = null;
    this.showhidetable = false;
    this.dataisempty = true;
    this.savebutton1 = 'Show';
  ...
  BindCalendarYear() {
    this._employeeAttendService.GetCalendarYear(this._commonService.getschemaname()).subscribe((res:any) => {
      if (res != null) {
  ...
  BindCalendarMonth() {
    ...
    this._employeeAttendService.GetTDSJVCalendarYearMonth(this.CalendarId,
      this._commonService.getschemaname()
    ).subscribe((res:any) => {
      if (res != null) {
