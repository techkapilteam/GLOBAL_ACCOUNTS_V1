...
export class CashOnhandComponent implements OnInit {
  ...
  cashbalance: any;
  caobranchid: any;
  bankDropdowmHide: boolean = false;
  ...
  constructor(private fb: FormBuilder, private _accountingtransaction: AccountingTransactionsService, private _commonService: CommonService, private datepipe: DatePipe,
    ...
    // this.todate.containerClass = this._commonService.datePickerPropertiesSetup('containerClass');
    // this.todate.showWeekNumbers = this._commonService.datePickerPropertiesSetup('showWeekNumbers');
    // this.todate.maxDate = new Date();
    // this.todate.dateInputFormat = this._commonService.datePickerPropertiesSetup('dateInputFormat');
  ...
  ngOnInit(): void {
    ...
    // })
    this._accountingtransaction.GetCAOBranchList('global','accounts','KAPILCHITS','KLC01').subscribe(bankslist => {
      this.BanksList = bankslist;
    })
  ...
  BankChange(event: any) {
    ...
    this.selectedBankid = this.selectedBankdetails.paccountid;
  ...
  checkedDeposit(event: any, data: any) {
    ...
    if (event.target.checked == true) {
      ...
      // }
      if (receiptdate !== null && transactiondate >= receiptdate) {
        data.pdepositstatus = true;
        data.pcancelstatus = false;
        data.pchequestatus = "P";
      }
    ...
    let bool1 = this.gridData.every((res: any) => res.pdepositstatus == false);
    if (bool1) {
    ...
    let bool2 = this.gridData.some((res: any) => res.pdepositstatus == false);
    if (bool2) {
      this.allrowsSelected = false;
    }
  }
  ...
  GetValidationByControl(formGroup: FormGroup, key: string, isValid: boolean): boolean {
    try {
      ...
      // If it's a nested FormGroup, recurse
      if (formcontrol instanceof FormGroup) {
        return this.checkValidations(formcontrol, isValid);
      }

      // If the control has validators
      if (formcontrol.validator) {
        // Initialize the error message container
        if (!this.CashOnHandValidation[key]) {
          this.CashOnHandValidation[key] = '';
        } else {
          this.CashOnHandValidation[key] = ''; // clear previous messages
        }

        // Only proceed if control has errors or is invalid
        if (formcontrol.errors || formcontrol.invalid || formcontrol.touched || formcontrol.dirty) {
          let lablename: string = '';
          if (key !== 'fromdate') {
            const element = document.getElementById(key) as HTMLInputElement;
            lablename = element?.title ?? '';
          }
          ...
          errorKeys.forEach(errorkey => {
            const errormessage = this._commonService.getValidationMessage(
              formcontrol,
              errorkey,
              lablename, // now guaranteed string
              key,
              ''
            );
            this.CashOnHandValidation[key] += errormessage + ' ';
  ...
  DownloadJVListDetails(fromdate: any, todate: any, mvnumber: any, formname: any) {
    debugger;
    this.jvlistDatanew = [];
    this.AccountingReportsService.GetSubscriberJvListReport(fromdate, todate, mvnumber, formname).subscribe(res => {
      if (res) {
  ...
  pdfOrprintnewJvst(printorpdf: any, formname: any, fromdate: any, todate: any) {
    debugger
    let rows: any = [];
   ...
   let reportType: "Between" | "As On" = "Between"; // assign based on your logic

this._commonService._JvListdownloadReportsPdf(
    reportname,
    rows,
    gridheaders,
    colWidthHeight,
