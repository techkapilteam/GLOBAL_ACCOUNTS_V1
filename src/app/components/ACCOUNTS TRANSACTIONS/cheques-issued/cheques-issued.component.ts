...
export class ChequesIssuedComponent implements OnInit {
  ...
  GetChequesIssued_Load(bankid: number) {
    ...
    forkJoin([
      ...
      // ─── FIXED: GetChequesIssuedData → mapped to GetChequesIssued service call ───
      // Service signature: GetChequesIssued(_BankId, BrsFromDate, BrsTodate, BranchSchema,
      ...
      // ─── FIXED: GetChequesRowCount uses correct parameter order ───
      // Service signature: GetChequesRowCount(depositedBankId, GlobalSchema, BranchSchema,
      //                                       searchtext, BrsFromDate, BrsTodate,
      //                                       formname, modeofreceipt, companyCode, branchCode)
      this._accountingtransaction.GetChequesRowCount(
        bankid,             // depositedBankId
        GlobalSchema,       // GlobalSchema
        BranchSchema,       // BranchSchema
        this._searchText,   // searchtext
        this.fromdate,      // BrsFromDate
        this.todate,        // BrsTodate
        'CHEQUESISSUED',    // formname
        this.modeofreceipt, // modeofreceipt
        CompanyCode,        // companyCode
        BranchCode          // branchCode
      )
  ...
  GetChequesIssued(bankid: any, startindex: number, endindex: number, searchText: string) {
    ...
    // ─── FIXED: Replaced GetChequesIssuedData with GetChequesIssued ───
    // Service signature: GetChequesIssued(_BankId, BrsFromDate, BrsTodate, BranchSchema,
    //                                     startindex, endindex, modeofreceipt, searchtext,
    //                                     GlobalSchema, branchcode, companycode)
    this._accountingtransaction
      ...
      .subscribe({
        ...
        next: (data: any) => {
          ...
          switch (this.status) {
            case 'all': this.All1(); break;
            case 'chequesissued': this.ChequesIssued1(); break;
            case 'onlinepayment': this.OnlinePayments1(); break;
            case 'cleared': this.Cleared1(); break;
            case 'returned': this.Returned1(); break;
            case 'cancelled': this.Cancelled1(); break;
          }
  ...
  GetDataOnBrsDates(frombrsdate: any, tobrsdate: any, bankid: any) {
    ...
    // ─── FIXED: Both calls now use correct parameter order ───
    ...
    const count$ = this._accountingtransaction.GetChequesRowCount(
      ...
      BranchSchema,     // BranchSchema
      this._searchText, // searchtext
      frombrsdate,      // BrsFromDate
      tobrsdate,        // BrsTodate
      'CHEQUESISSUED',  // formname
      '',               // modeofreceipt (empty for BRS date range query)
      CompanyCode,      // companyCode
      BranchCode        // branchCode
    );
  ...
  pdfOrprint(printorpdf: 'pdf' | 'print') {
    ...
    forkJoin([
      // ─── FIXED: PDF export also uses correct GetChequesIssued signature ───
      this._accountingtransaction.GetChequesIssued(
  ...
  export() {
    ...
    forkJoin([
      // ─── FIXED: Excel export also uses correct GetChequesIssued signature ───
      this._accountingtransaction.GetChequesIssued(
        this.bankid, '', '', BranchSchema,
        0, 999999, this.modeofreceipt, this._searchText,
        GlobalSchema, BranchCode, CompanyCode
